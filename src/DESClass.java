...
public class DESClass {
    ...
    interface tables {
       byte [] PC1 = new byte[]{57,49,41,33,25,17,9,1,
       ...
       byte [] shiftValue = new byte[]{1,1,2,2,2,2,2,2,1,2,2,2,2,2,2,1};
      
       byte[] PC2 = new byte[]{14,17,11,24,1,5,
                               3,28,15,6,21,10,
                               23,19,12,4,26,8,
                               16,7,27,20,13,2,
                               41,52,31,37,47,55,
       ...
       byte [] IP = new byte[]{58,50,42,34,26,18,10,2,
       ...
       byte [] FP = new byte []{40,8,48,16,56,24,64,32,
       ...
       //This is Expansion
       byte [] E_BOX = new byte[]{32,1,2,3,4,5,
                                  4,5,6,7,8,9,
                                  8,9,10,11,12,13,
                                  12,13,14,15,16,17,
                                  16,17,18,19,20,21,
       ...
       byte [][] SBOX = new byte[][]{
                                     {14,4,13,1,2,15,11,8,3,10,6,12,5,9,0,7,
                                      ...
                                      15,12,8,2,4,9,1,7,5,11,3,4,10,0,6,13},
    ...
    public  String encryptMessage(String plain, String key1)
    {
     ...
     for (int i=0; i<plain.length(); i+= 64)
      {
        plain_block = encrypt(plain.substring(i, i+64));
        builder.append(fromBinaryToString(plain_block));
      }
     return builder.toString();
    }
    public  String decryptMessage(String cipher, String key1){
        generateSubKeys(key1);
        StringBuilder builder = new StringBuilder();
        String cipher_block;
        cipher = toBinary(cipher);
        for(int i = 0; i <cipher.length(); i+=64)
        {
             cipher_block = decrypt(cipher.substring(i,i+ 64));
             builder.append(fromBinaryToString(cipher_block));
        }
       return builder.toString();
    }
    ...
    private  void generateSubKeys(String key1)
    {
        ...
        {   
            ...
            for(int i = 0; i< 16; i++)
            {
               newLeft = shift(prevLeft, tables.shiftValue[i]);
               prevLeft = newLeft;
               newRight = shift(prevRight, tables.shiftValue[i]);
               prevRight = newRight;
               newSubKey = newLeft + newRight;
               newSubKey = permute(newSubKey, tables.PC2, 56);
    ...
    public  String encrypt(String plain)
    {
        ...
        {
          plain = permute(plain, tables.IP, 64);
          if(plain==null)
          {
              return plain;
          }
         ...
         String finalCipher = permute(cipher, tables.FP, 64);
         return finalCipher;
        }
        else
        {
    ...
    public  String decrypt(String cipher)
    {
        if(subKeys != null && subKeys.size()==16)
        {
          cipher = permute(cipher, tables.IP, 64);
          if(cipher == null)
          {
            return cipher;
          }
         ...
         String plain = left + right;
         String finalPlain = permute(plain, tables.FP, 64);
         return finalPlain;
        }
        else
        {
    ...
    public static String fromBinaryToString(String a)
    {
        if(a.length()%8 ==0)
        {
          StringBuilder builder = new StringBuilder();
          ...
          {
           temp = Integer.parseInt(a.substring(i, i + 8),2);
    ...
    private  void Ffunction(String key)
    {
       String newRight = expandWithEBox();
       newRight = XOR(newRight, key);
       newRight = sbox(newRight);
       newRight = permute(newRight, tables.P, 32);
    ...
    private  String shift(String input, byte b)
    {
      ...
      //return input.substring(b)+ input.substring(0, b);
    ...
    private String expandWithEBox() {
        StringBuilder builder = new StringBuilder();
        for (byte index : tables.E_BOX) {
